...
function parseMeta(meta) {
    ...
    /*
     ...
     /*const parseChildren = (children, parentPath, parentRealPath) => {
        ...
        if ((typeof propValue === 'undefined' ? 'undefined' : (0, _typeof3.default)(propValue)) == 'object' && propValue.get && propValue.get('name') && propValue.get('component')) {
            var path = parentPath ? parentPath + '.' + propValue.get('name') : propValue.get('name');
                ...
                if (p == 'children') {
                    if (v && typeof v != 'string') {
                        v.forEach(function (child, index) {
                            var currentRealPath = parentRealPath ? parentRealPath + '.children.' + index : 'children.' + index;
                            parseProp(child, path, currentRealPath);
                        });
                    }
                } else {
                    if (p == 'columns') debugger;
