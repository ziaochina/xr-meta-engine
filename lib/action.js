...
var _initialiseProps = function _initialiseProps() {
	...
	this.updateChildrenMeta = function (childrenMeta, parentPath, rowIndex, vars) {
	...
	this.updateMeta = function (meta, path, rowIndex, vars) {
		(0, _keys2.default)(meta).forEach(function (key) {
			//$$getVisible
			if (typeof meta[key] == 'string' && meta[key].substring(0, 2) === '$$') {
				if (!_this.metaHandlers || !_this.metaHandlers[meta[key].substring(2)]) throw 'not found ' + meta[key] + ' handler, please define in action';

				var handlerName = meta[key].substr(2);
				meta[key] = function () {
					_this.metaHandlers[handlerName]({ path: path, rowIndex: rowIndex, vars: vars });
				};
				meta[key] = meta[key].bind(_this);
			}

			//##form.code
			if (typeof meta[key] == 'string' && meta[key].substring(0, 2) === '##') {
				meta[key] = _this.getField(_this.calcBindField(meta[key].substr(2), vars));
			...
			if (typeof meta[key] == 'string' && meta[key].indexOf('->') !== -1) {
				...
				var f = new Function('args', 'fieldPath', 'setField', 'setField(fieldPath, ' + tmp[0] + ')').bind(_this);
				meta[key] = function () {
					for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
						args[_key] = arguments[_key];
					}

					f(args, _this.calcBindField(tmp[1], vars), _this.setField);
				};
			...
			if ((0, _typeof3.default)(meta[key]) == 'object' && meta[key].component) {
				_this.updateMeta(meta[key], path + '.' + key, rowIndex, vars);
			}

			if (key === 'children') _this.updateChildrenMeta(meta[key], path, rowIndex, vars
   ...
   if(key === 'bindField'){
	...
	this.getMeta = function (fullPath, propertys) {
		...
		_this.updateMeta(meta, path, rowIndex, vars);
		return meta;
	};
